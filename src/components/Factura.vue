...
const agregarProducto = () => {
  if (productoSeleccionado.value && cantidad.value && precio.value) {
    ...
    if (productoExistente) {
      productoExistente.cantidad += Number(cantidad.value); // Actualizar cantidad
      productoExistente.subtotal =
        productoExistente.precio * productoExistente.cantidad; // Actualizar subtotal
    } else {
...
const crearFactura = async () => {
  if (nuevaFactura.value.productosSeleccionados.length === 0) {
    alert("Debe agregar al menos un producto para crear la factura");
    return;
  }
...
const EditarFactura = async () => {
  ...
  try {
    ...
    alert("Error al editar la factura, por favor intente nuevamente.");
  }
};
